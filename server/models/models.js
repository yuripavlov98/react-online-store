...
const User = sequelize.define('user', {
    id: {type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true}, // числовой id
...
User.hasOne(Basket) // Пользователь -> Корзина = связь односторонняя -> hasOne
Basket.belongsTo(User) // Корзина принадлежит пользователю = belongsTo

User.hasMany(Rating) // Пользователь может ставить неск оценок -> hasMany
Rating.belongsTo(User) // Поставленные оценки принадлежат одному пользователю = belongsTo
...
Type.belongsToMany(Brand, {through: TypeBrand}) // Связь многие ко многим -> belongsToMany, нужна {промежуточная позиция}
